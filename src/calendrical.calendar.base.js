var Calendrical = (function (exports) {
    ...
    calendar.updateFromGregorian = function () {
        ...
        jd = this.gregorianToJd(year, mon + 1, day) +
            (Math.floor(sec + 60 * (min + 60 * hour) + 0.5) /
                86400.0);
    ...
    calendar.updateHebrew = function (jd) {
        ...
        data.hebrew = {
            ...
            day: hebcal[2],
            hebmonth: this.constants.hebrew.H_MONTHS[hebcal[1] -
                1]
        }
    ...
    calendar.updateIslamic = function (jd) {
        ...
        data.islamic = {
            ...
            day: islcal[2],
            wday: "yawm " + this.constants.islamic.WEEKDAYS[
                astro.jwday(jd)],
    ...
    calendar.updatePersian = function (jd) {
        ...
        data.persian = {
            ...
            day: perscal[2],
            wday: this.constants.persian.WEEKDAYS[astro.jwday(
                jd)],
    ...
    calendar.updateMayan = function (jd) {
        ...
        data.mayan_count = {
            ...
            uinal: mayancal[3],
            kin: mayancal[4],
            haab: mayhaabcal[1] + " " + this.constants.mayan.HAAB_MONTHS[
                mayhaabcal[0] - 1],
            tzolkin: maytzolkincal[1] + " " + this.constants.mayan
                .TZOLKIN_MONTHS[maytzolkincal[0] - 1]
        }
    ...
    calendar.updateBahai = function (jd) {
        var bahcal = this.jdToBahai(jd),
            bahaiYear = ((((bahcal[0] - 1) * 19) + bahcal[1] - 1) *
                19) + bahcal[2] - 1,
            leap;
        // Bahai uses same leap rule as Gregorian until 171 Bahai Era
        // From 172 onwards, it uses the Persian leap year algorithm
        if (bahaiYear < 172) {
            leap = this.leapGregorian(this.jdToGregorianYear(jd));
        } else {
            leap = this.leapPersian(1222 + bahaiYear);
        }
        ...
        data.bahai = {
            ...
            weekday: this.constants.bahai.WEEKDAYS[astro.jwday(
                jd)],
            leap: leap
        }
    ...
    calendar.updateIndianCivil = function (jd) {
        ...
        data.indian_civil = {
            year: indcal[0],
            month: this.constants.indian_civil.MONTHS[indcal[1] -
                1],
            day: indcal[2],
            weekday: this.constants.indian_civil.WEEKDAYS[astro
                .jwday(jd)],
    ...
    calendar.updateFrenchRevolutionary = function (jd) {
        ...
        data.french = {
            an: frrcal[0],
            mois: this.constants.french_revolutionary.MOIS[
                frrcal[1] - 1],
            decade: this.constants.french_revolutionary.DECADE[
                frrcal[2] - 1],
            jour: this.constants.french_revolutionary.JOUR[((
                frrcal[1] <= 12) ? frrcal[3] : (
                frrcal[3] + 11)) - 1]
        }
    ...
    calendar.updateExcel1900 = function (jd) {
        data.excel_serial_1900.day = (jd - this.constants.J1900) +
            1 +
    ...
    calendar.updateUnixTime = function (jd) {
        var utime = (jd - this.constants.J1970) * (60 * 60 * 24 *
            1000);
    ...
    calendar.augmentGregorian = function (jd) {
        data.gregorian.wday = astro.constants.WEEKDAYS[astro.jwday(
            jd)];
